 ...
 */
onion *onion_new_with_parm(int flags, char *hostname, char *port){
...
onion *onion_new(int flags){
	return onion_new_with_parm (flags, NULL, NULL);
}
...
int onion_listen(onion *o){
	...
	if (sockfd==0){
		...
		if ((ret = getaddrinfo(o->hostname, o->port, &hints, &result)) != 0 ){
			ONION_ERROR("Error getting local address and port: %s(%d)", strerror(errno), ret);
			return errno;
		}
